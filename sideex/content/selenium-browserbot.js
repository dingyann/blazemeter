...
BrowserBot.prototype.findElementOrNull = function(locator, win) {
    if (locator.startsWith('cache=')) {
        const cache = bot.inject.cache.getCache_();
        const el = cache[locator.substring(6)];
...
BrowserBot.prototype.isElementClickable = function(elem, coord) {
    /**
...
BrowserBot.prototype.isShown = function(element, ignoreOpacity) {
    return bot.dom.isShown(element, ignoreOpacity);
};
