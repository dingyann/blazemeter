...
Selenium.prototype.doAssertText = function(locator, value) {
    var element = this.browserbot.findElement(locator);
    var isShown = this.browserbot.isShown(element, true);

    if (!isShown) {
        throw new Error(`Element ${locator} is expected to be visible`);
    }
...
Selenium.prototype.doStore = function(varName, value) {
...
Selenium.prototype.doWaitFor = function(locator, condition) {
    let element;
    try {
        element = this.browserbot.findElement(locator);
    } catch (e) {
        if (condition === 'present' || condition === 'clickable' || condition === 'visible') {
            throw new SeleniumError(`Element ${locator} is expected to be present`);
        }
        return;
    }

    if (condition === 'not_present' && element) {
        throw new SeleniumError(`Element ${locator} is not expected to be present`);
    }

    const isShown = this.browserbot.isShown(element, /*ignore_opacity*/true);

    if (condition === 'visible' && !isShown) {
        throw new SeleniumError(`Element ${locator} is expected to be visible`);
    }

    if (condition === 'not_visible' && isShown) {
        throw new SeleniumError(`Element ${locator} is not expected to be visible`);
    }

    const rect = element.getBoundingClientRect();
    const result = this.browserbot.isElementClickable(element, {x: rect.x, y: rect.y});
    const clickable = result.clickable && !element.disabled;

    if (condition === 'clickable' && !clickable) {
        throw new SeleniumError(`Element ${locator} is expected to be clickable`);
    }

    if (condition === 'not_clickable' && clickable) {
        throw new SeleniumError(`Element ${locator} is not expected to be clickable`);
    }
};
...
Selenium.prototype.doResizeWindow = function (_url, windowSize) {
    const [width, height] = windowSize.split(',').map(Number);
    const window = this.browserbot.getCurrentWindow();
    const HEIGHT_OFFSET = 78;
    ...
    if (window != null && window.screen) {
        ...
        const messageOptions = {
            command: 'changeWindowSize',
            value: [width, height + HEIGHT_OFFSET]
        };
        chrome.runtime.sendMessage(messageOptions);
        window.resizeTo(width, height);
    }
};

Selenium.prototype.doMaximizeWindow = function() {
    const  window = this.browserbot.getCurrentWindow();
...
Selenium.prototype.doAssertDialog = function(type, value) {
    switch (type.toLowerCase()) {
        case 'alert':
            return this.doAssertAlert(value);
        case 'confirm':
            return this.doAssertConfirmation(value);
        case 'prompt':
            return this.doAssertPrompt(value);
        default:
            throw new SeleniumError('Invalid dialog type provided');

    }
};

Selenium.prototype.doAnswerDialog = function(type, value) {
    const response = value && value.toLowerCase();
    switch (type.toLowerCase()) {
        case 'alert':
            return;
        case 'confirm':
            if (response === '#ok') {
                return this.doChooseOkOnNextConfirmation(response);
            } else {
                return this.doChooseCancelOnNextConfirmation();
            }
        case 'prompt':
            if (response === '#cancel') {
                return this.doChooseCancelOnNextConfirmation();
            } else {
                return this.doAnswerOnNextPrompt(value);
            }
        default:
            throw new SeleniumError('Invalid dialog type provided');
    }
...
Selenium.prototype.doEval = function(script) {
    let result = this.getEval(script);
    if (result === 'null') {
        result = false;
    }
...
Selenium.prototype.doAssertEval = function(script) {
    let result = this.getEval(script);
    // getEval changes null value to be string with text 'null'
    if (result === 'null') {
        result = false;
    }

    if (!result) {
        throw new Error("Assertion failed. Got result: " + result);
    }
};

Selenium.prototype.doStoreEval = function(varName, script) {
    const result = this.getEval(script);
    browser.runtime.sendMessage({ "storeStr": result, "storeVar": varName });
};

Selenium.prototype.doFindElements = function(locator, root) {
    if (typeof root === 'string' && root.startsWith('cache=')) {
        root = this.browserbot.findElement(root);
    }
    const result = this.browserbot.findElementsLikeWebDriver(locator.name.toLowerCase(), locator.value, root);
    if (!result) {
