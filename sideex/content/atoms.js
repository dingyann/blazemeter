...
goog.dom.selection.setText = function(a, b) {
    const typeEmail = a.type === 'email';
    const typeNumber = a.type === 'number';
    if (typeEmail || typeNumber) {
        a.type = 'text';
        a.selectionStart = a.value.length;
        a.selectionEnd = a.value.length;
    }
    a.type = typeEmail || typeNumber ? 'text' : a.type;

    const some = goog.dom.selection.useSelectionProperties_(a);
    if (goog.dom.selection.useSelectionProperties_(a)) {
        var c = a.value,
            d = a.selectionStart,
            e = c.substr(0, d),
...
core.events.replaceText_ = function(a, b) {
    ...
    bot.dom.isElement(a, goog.dom.TagName.BODY) ? a.ownerDocument && a.ownerDocument.designMode && "on" == (String(a.ownerDocument.designMode)).toLowerCase() && (a.innerHTML = d) : goog.userAgent.GECKO && bot.userAgent.FIREFOX_EXTENSION && bot.userAgent.isEngineVersion(8) ? XPCNativeWrapper(a).value = d : a.value = d;
    try {
        bot.events.fire(a, bot.events.EventType.CHANGE);
    } catch (e) {}
};
core.events.setValue = function(a, b) {
    ...
    var c = core.locators.findElement(a),
        d = core.events.shiftKeyDown_ ? (String(b)).toUpperCase() : b;
