...
class ExtCommand {
    ...
    constructor(contentWindowId) {
        ...
        this.tabsOnUpdatedHandler = (tabId, changeInfo) => {
    ...
    // Used dynamically in playback-api.js
    ...
    // Used dynamically in playback-api.js
    // noinspection JSUnusedGlobalSymbols
    doPause(ignored, time) {
        // Logic taken from https://github.com/Blazemeter/taurus/blob/604100b26bb843bc4daae9029a0c225715d79f5b/bzt/utils.py#L162
        function dehumanizeTime(time) {
            const re = /([\d.\-infa]+)([a-zA-Z]*)/gi;
            const parts = [...time.matchAll(re)];

            let result = 0;

            for (const [__, value, unit] of parts) {
                if (unit === 'ms') {
                    result += Number(value) / 1000.0;
                } else if (!unit || unit === 's') {
                    result += Number(value);
                } else  if (unit === 'm') {
        ...
        return new Promise(function (resolve) {
            setTimeout(resolve, dehumanizeTime(time));
        });
    }
    ...
    doSelectWindow(serialNumber) {
        let self = this;
        browser.runtime.sendMessage({command: "get_debuggerTab"}).then((response) => {
            browser.windows.update(response.debuggerTab.windowId, {focused: true});
        });
        //Temporal fix for the selectWindow bug, while interacting on an iframe
        this.currentPlayingFrameLocation = "root";
    ...
    // noinspection JSUnusedGlobalSymbols
    doOpenWindow(url) {
        const tabUrl = url === '""' ? 'chrome://newtab/' : url;
        let self = this;
        return Promise.resolve(chrome.tabs.create({'url': tabUrl}, function(tab) {
            chrome.tabs.update(tab.id, {active: true});
            self.currentPlayingTabId = tab.id;
            self.setNewTab(tab.id);
        }));
    }
    ...
    isAddOnPage(url) {
        return url.startsWith("https://addons.mozilla.org")
            || url.startsWith("https://chrome.google.com/webstore");
    }

    static isExtCommand(command) {
        switch (command) {
            case "pause":
            case "open":
            case "selectFrame":
